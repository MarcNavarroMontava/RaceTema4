...
```{r}
# Cargar el fichero binario
load(file = './data/2024-03-12.Rdata')
dim(mydf)
head(mydf)

# lo transformamos en un tibble
cran <- as_tibble(mydf)
...
# Usamos glimpse para obtener un resumen de la estructura de los datos
cran %>% glimpse()
...
```{r}
# Seleccionamos solo las columnas ip_id, package y country
...
```{r}
# Seleccionamos todas las columnas entre r_arch y country
...
```{r}
# Filtramos los registros donde se descargó el paquete 'plotly' y la versión de R es '3.5.1'
...
```{r}
# Filtramos las descargas realizadas desde España (ES) con versiones de R superiores a 3.4.0
...
```{r}
# Filtramos las descargas realizadas desde España (ES) o Portugal (PT)
...
```{r}
# Filtramos las descargas con tamaño superior a 100500 bytes desde un sistema operativo 'linux-gnu'
cran %>% filter(size > 100500 & system == "linux-gnu")
...
```{r}
# Filtramos las descargas donde la versión del sistema operativo no está vacía
cran %>% filter(!is.na(version))
...
```{r}
# Almacenamos en cran2 todas las columnas entre 'size' y 'ip_id'
...
```{r}
# Ordenamos cran2 en orden ascendente según la variable ip_id
cran2 %>% arrange(ip_id)
...
```{r}
# Ordenamos cran2 en orden descendente según la variable ip_id
...
```{r}
# Ordenamos cran2 según los valores de package e ip_id
...
```{r}
# Ordenamos cran2 según country (ascendente), r_version (descendente) e ip_id (ascendente)
...
```{r}
# Seleccionamos las columnas ip_id, package y size y las almacenamos en cran3
...
```{r}
# Creamos una nueva variable 'size_mb' en cran3, convirtiendo 'size' de bytes a Mbytes
...
```{r}
# Creamos una nueva variable 'size_gb' en cran3, convirtiendo 'size' de bytes a Gbytes
cran3 <- cran3 %>% mutate(size_gb = size / (2^10 * 2^20))
...
```{r}
# Creamos una nueva variable 'correct_size' ajustando el tamaño en 1000 bytes
...
```{r}
# Calculamos el valor medio del tamaño de las descargas y lo almacenamos en avg_bytes
...
```{r}
# Creamos un agrupamiento por 'package' en cran y lo almacenamos en 'by_package'
...
```{r}
# Calculamos el valor medio de 'size' por cada 'package' y mostramos el resultado
by_package %>% summarise(mean_size = mean(size, na.rm = TRUE))
...
```{r}
# Calculamos los valores requeridos y los almacenamos en 'pack_sum'
pack_sum <- by_package %>%
  summarise(
    count = n(),
    unique = n_distinct(ip_id),
...
```{r}
# Calculamos el umbral del 1% de los paquetes más descargados
top_counts <- cran %>%
  group_by(package) %>%
  summarise(count = n()) %>%
  filter(count > quantile(count, probs = 0.99))  # Filtramos los paquetes con más descargas
...
```{r}
# Calculamos el umbral del 1% y ordenamos en orden descendente por el número de descargas
top_counts_sorted <- cran %>%
  group_by(package) %>%
  summarise(count = n()) %>%
  filter(count > quantile(count, probs = 0.99)) %>%
  arrange(desc(count))  # Ordenamos por el número de descargas en orden descendente
...
```{r}
# Calculamos el número de ordenadores distintos y ordenamos por 'unique'
top_unique_sorted <- cran %>%
  group_by(package) %>%
  summarise(unique = n_distinct(ip_id)) %>%
  filter(unique > quantile(unique, probs = 0.99)) %>%
  arrange(desc(unique))  # Ordenamos por el número de ordenadores distintos en orden descendente
...
```{r}
# Seleccionamos las columnas ip_id, country, package y size y mostramos el resultado
cran %>%
...
```{r}
# Añadimos la columna 'size_mb' con el tamaño de cada descarga en Megabytes
...
```{r}
# Seleccionamos las descargas cuyo tamaño es menor o igual que 0.5 Mbytes
...
```{r}
# Seleccionamos las descargas cuyo tamaño es menor o igual que 0.5 Mbytes y las ordenamos en orden descendente
cran %>% filter(size_mb <= 0.5) %>% arrange(desc(size_mb)) %>% print()
...
# Transformamos el data frame 'students' en un conjunto tidy
...
```{r}
# Transformamos 'students2' en un conjunto tidy
students2_tidy <- students2 %>%
  gather(key = "sex_class", value = "count", male_1, female_1, male_2, female_2) %>%
...
```{r}
# Transformamos 'students3' en un conjunto tidy
students3_tidy <- students3 %>%
  gather(key = "class", value = "score", class1, class2, class3, class4, class5) %>%
  mutate(class = gsub("class", "", class)) # Extraemos el número de la clase
...
```{r}
# Convertimos 'class1', 'class2', etc., a números
students4 <- students4 %>%
  mutate(class = parse_number(names(students4)[grep("class", names(students4))]))
...
```{r}
# Separar 'students4' en dos tablas
student_info <- students4 %>%
  select(id, name, class)

gradebook <- students4 %>%
  select(id, midterm, final)
...
```{r}
# Añadimos la columna 'status' y unimos los datos
failed %>%
...
```{r}
# Imaginamos un conjunto sat
# Vamos a convertir los datos a formato tidy.

sat_tidy <- sat %>%
  gather(key = "score_range", value = "count", -part, -sex) %>%  # Reorganizamos las columnas en formato largo
  select(score_range, part, sex, count)  # Seleccionamos las columnas en el orden correcto
...
```{r}
# Agrupamos los datos por 'part' y 'sex' y creamos las columnas 'total' y 'prop'
sat_tidy_summary <- sat_tidy %>%
  group_by(part, sex) %>%  # Agrupamos por 'part' y 'sex'
  mutate(
    total = sum(count),  # Calculamos el total de estudiantes por grupo
    prop = count / total  # Calculamos la proporción dentro de cada grupo
  ) %>%
  ungroup()  # Desagrupamos después de realizar los cálculos
...
```{r}
# Cargamos la librería ggplot2 para la visualización
library(ggplot2)

# Graficamos las diferencias entre hombres y mujeres en las diferentes pruebas para cada rango de puntuación
ggplot(sat_tidy_summary, aes(x = score_range, y = prop, fill = sex)) +
  geom_bar(stat = "identity", position = "dodge") +  # Barras separadas para hombres y mujeres
  facet_wrap(~ part) +  # Facetas por parte del examen
  labs(
    title = "Proporción de estudiantes por sexo y rango de puntuación en el SAT",
